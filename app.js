...
function signin(email, password) {
  if (!email || !password)
    return { success: false, message: "Введіть email та пароль" };
...
const DEFAULT_CATEGORIES = {
  "r/ігри": {
    ...
    color: "#ff4500",
    flair: ["Ігри", "flair-cs"],
    desc: "Обговорення відеоігор",
  },
  "r/технології": {
    emoji: "💻",
    color: "#58a6ff",
    flair: ["Технології", "flair-tech"],
    desc: "IT, програмування, гаджети",
  },
  "r/навчання": {
    emoji: "📚",
    color: "#3fb950",
    flair: ["Навчання", "flair-tech"],
    desc: "Освіта і саморозвиток",
  },
  "r/спорт": {
    emoji: "⚽",
    color: "#fb8c00",
    flair: ["Спорт", "flair-news"],
    desc: "Спортивні новини та події",
  },
  "r/фільми": {
    emoji: "🎬",
    color: "#ab47bc",
    flair: ["Фільми", "flair-cs"],
    desc: "Кіно, серіали, огляди",
  },
};
...
const SEED_POSTS = [
  {
    ...
    flairClass: "flair-cs",
    title:
      "Olofmeister злякався після матчу проти ZywOo, m0NESY, donk, NiKo & YEKINDAR 1 квітня",
    emoji: "🎮",
    votes: 4700,
    comments: 214,
    body: "Легенда CS:GO потрапила у матч мрії — і виглядало це дуже кумедно.",
  },
  {
    id: 1001,
    authorId: null,
    ...
    time: "5 год тому",
    flair: "Технології",
    flairClass: "flair-tech",
    title: "React 20 офіційно вийшов — нові хуки та покращена продуктивність",
    emoji: "💻",
    votes: 2100,
    comments: 89,
    body: "Meta випустила React 20 з революційними змінами.",
  },
  {
    id: 1002,
    authorId: null,
    authorName: "study_olya",
    sub: "r/навчання",
    subColor: "#3fb950",
    time: "1 день тому",
    flair: "Навчання",
    ...
    votes: 891,
    comments: 143,
    body: "Пройшла шлях від нуля до Middle: Python → TypeScript.",
  },
  {
    id: 1003,
    authorId: null,
    authorName: "footbal_ivan",
    sub: "r/спорт",
    subColor: "#fb8c00",
    time: "2 дні тому",
    flair: "Спорт",
    ...
    votes: 3420,
    comments: 512,
    body: "До фіналу залишилось менше тижня.",
  },
  {
    id: 1004,
    authorId: null,
    authorName: "kino_luda",
    sub: "r/фільми",
    subColor: "#ab47bc",
    time: "4 дні тому",
    flair: "Фільми",
    ...
    votes: 1560,
    comments: 77,
    body: "Переглянула понад 80 фільмів цього року.",
  },
];
...
function savePosts(list) {
  localStorage.setItem("goat_posts", JSON.stringify(list));
}
...
function doSignin() {
  ...
  renderHeader();
  if (
    document.getElementById("page-post").classList.contains("active") &&
    currentPostId !== null
  )
...
function renderProfile(tab) {
  ...
  const emptyMap = {
    ...
    saved: {
      icon: "🔖",
      title: "Збережених постів немає",
      sub: "Натисніть «Зберегти» під будь-яким постом.",
    },
    upvoted: { icon: "⭐", title: "Вподобаних немає", sub: "Голосуйте вгору." },
    downvoted: {
      icon: "👎",
      title: "Не вподобаних немає",
      sub: "Голосуйте вниз.",
    },
  };
...
function postComment(postId) {
  ...
  if (header)
    header.textContent = `💬 Коментарі (${postComments[postId].length})`;
  if (list)
    list.innerHTML = postComments[postId].map((c) => renderComment(c)).join("");
  renderHeader();
...
function saveCat() {
  ...
  if (editKey && editKey !== name) {
    ...
    if (hasPosts) {
      showToast("Не можна змінити slug — є пости в цій категорії!", "warn");
      return;
    }
    delete cats[editKey];
  }
  ...
  renderFeed();
  showToast(
    editKey ? "✅ Категорію оновлено!" : "✅ Категорію створено!",
    "success",
  );
}
...
function renderAdminUsers() {
  ...
  document.getElementById("userCount").textContent =
    `Всього: ${users.length} користувачів`;
...
function toggleJoin(btn) {
  ...
  showToast(j ? "✅ Ви приєднались" : "Ви покинули спільноту", "success");
}
